...
public class new_mesh : MonoBehaviour
{
    ...
    void Start()
    {
        Mesh mesh = CreateMesh(vertices, vertices, size);
    ...
    private Mesh CreateMesh(int xCount, int yCount, float size)
    {
        Mesh mesh = new Mesh();
        ...
        for (int i = 0, y = 0; y <= yCount; y++)
        {
            ...
            {
                vertices[i] = new Vector3((float)x / xCount * size, 0, (float)y /
                yCount * size);
